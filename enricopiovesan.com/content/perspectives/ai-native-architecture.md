...
These properties correspond to a specific framework: [Contract-Driven AI Development (<abbr title="Contract-Driven AI Development">C-DAD</abbr>)](/c-dad/). They also require that the underlying capabilities be portable, invocable from any agent runtime without rewriting the logic. That portability is what [Universal Microservices Architecture (<abbr title="Universal Microservices Architecture">UMA</abbr>)](/uma/) addresses.
