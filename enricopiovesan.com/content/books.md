...
description: Two books by Enrico Piovesan on software architecture and AI-native systems. Universal Microservices Architecture published by Apress. The Day After AI forthcoming from Apress, making software companies legible to AI agents.
layout: books.njk
toc:
...
books:
  ...
  - title: "The Day After AI: Making Software Companies Legible to AI Agents"
    ...
    description:
      - Forthcoming from Apress. Contract signed, manuscript complete. Most software organizations are not ready for AI agents. Not because the models are immature. Because the codebases were never built to be navigable by anything other than the people who wrote them. Tribal knowledge is the bottleneck, not model capability.
      - The constraints live in Slack threads from two years ago. The intent is locked in the memory of engineers who may or may not still be at the company. The capability boundaries exist in someone's head, not in a capabilities graph the system can read. When an AI agent tries to work in that environment it does not fail loudly. It produces plausible output that is wrong in ways that take weeks to find.
      - The fix is not a rewrite. It extracts tribal knowledge into contracts role by role, capability by capability, instead of tearing the system down to rebuild it. The architect declares the reasoning space before the sprint begins. The developer owns a contract, not just a ticket. The PM asks in discovery what a story format was never designed to hold.
      - The Day After AI is a practical guide to that process, built from the same framework that produced the C-DAD white paper and the companion CLI tool, both developed since 2023. Full details and sample chapters are at thedayafteraibook.com.
---
